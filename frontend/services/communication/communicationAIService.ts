...
function factsBlock(ctx: CommunicationContext): string {
  ...
  if (ctx.outstandingBalance !== null) lines.push(`Outstanding balance: ${formatMoney(ctx.outstandingBalance)}`);
  ...
  if (focal) {
    ...
    if (focal.verificationUrl) lines.push(`Verification URL: ${focal.verificationUrl}`);
  }
  if (ctx.invoices.length > 1 && (ctx.purpose === 'payment_reminder' || ctx.purpose === 'outstanding_balance')) {
  ...
  if (ctx.lastPayment) lines.push(`Last payment: ${formatMoney(ctx.lastPayment.amount)} on ${ctx.lastPayment.date || 'unavailable'}${ctx.lastPayment.method ? ` via ${ctx.lastPayment.method}` : ''}`);
  if (ctx.quotation) lines.push(`Quotation: ${ctx.quotation.number} total ${formatMoney(ctx.quotation.total)}${ctx.quotation.validUntil ? ` valid until ${ctx.quotation.validUntil}` : ''}`);
  if (ctx.order) lines.push(`Order: ${ctx.order.id} total ${formatMoney(ctx.order.total)} status ${ctx.order.status}`);
  if (ctx.delivery) lines.push(`Delivery: ${ctx.delivery.id} status ${ctx.delivery.status}${ctx.delivery.trackingNumber ? ` tracking ${ctx.delivery.trackingNumber}` : ''}`);
  return lines.join('\n');
}
...
function purposeInstruction(ctx: CommunicationContext, customNote?: string): string {
  switch (ctx.purpose) {
    ...
    case 'payment_confirmation':
      return 'Write a thank-you payment confirmation stating the received amount and invoice reference where available.';
...
function deterministicFallback(ctx: CommunicationContext, opts: DraftOptions): string {
  ...
  switch (ctx.purpose) {
    ...
    case 'payment_confirmation':
      return `${greeting}, thank you! We have received your payment of ${ctx.lastPayment ? formatMoney(ctx.lastPayment.amount) : '[AMOUNT]'}${focal ? ` towards invoice ${focal.invoiceNumber}` : ''} at ${company}. We appreciate your business.`;
    case 'quotation_followup':
    ...
    case 'order_update':
      return `${greeting}, an update on your order ${ctx.order?.id || ''} at ${company}: status ${ctx.order?.status || 'in progress'}. We will notify you of any further changes.`;
    case 'delivery_notification':
      return `${greeting}, your delivery ${ctx.delivery?.id || ctx.order?.id || ''} from ${company} is ${ctx.delivery?.status || 'on its way'}${ctx.delivery?.trackingNumber ? ` (tracking ${ctx.delivery.trackingNumber})` : ''}. Thank you for your patience.`;
    case 'thank_you':
