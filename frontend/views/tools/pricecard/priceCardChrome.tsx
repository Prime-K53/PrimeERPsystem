...
/* ── Shared Price Cards chrome: exact Add-Customer modal language (mirrors ClientModal.tsx) ──
 * Single source of truth for the Smart Operations > Price Cards tab:
 * the tab page AND all Price Card modals must import from here — no local
 * token duplication. Tokens, overlay, accent stripe, icon-tile serif header,
 * styled form controls, KPI cards, table card, empty state and footer all
 * match views/sales/components/ClientModal.tsx.
 */
export const teal = {
...
export const ModalHeader: React.FC<{ icon: React.ReactNode; title: string; subtitle?: string; onClose: () => void }> = ({ icon, title, subtitle, onClose }) => (
  <div style={{
...
export const ModalFooter: React.FC<{ stepLabel?: string; onCancel: () => void; submitLabel: string; onSubmit?: () => void; submitDisabled?: boolean }> = ({ stepLabel, onCancel, submitLabel, onSubmit, submitDisabled }) => (
  <div style={{
    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: inkSoft, minWidth: 0 }}>
      ...
      <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{stepLabel || 'Price card ledger'}</span>
    </div>
    <div style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
      ...
      <button
        ...
        style={{ ...btnPrimaryStyle, opacity: submitDisabled ? 0.6 : 1, cursor: submitDisabled ? 'not-allowed' : 'pointer' }}
        onMouseEnter={e => { if (!submitDisabled) e.currentTarget.style.transform = 'translateY(-1px)'; }}
...
export const PageHeader: React.FC<{
  ...
  <div style={{
    ...
    padding: '22px 28px 18px',
    borderBottom: `1px solid ${hairline}`,
    background: paper, position: 'relative', gap: 16, flexWrap: 'wrap',
  }}>
...
export const SectionLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '4px 0 14px' }}>
    <span style={{
      fontSize: 11, fontWeight: 700, color: teal[800],
      textTransform: 'uppercase', letterSpacing: 0.1, whiteSpace: 'nowrap',
    }}>
      {children}
    </span>
