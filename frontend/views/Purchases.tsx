...
import { OrderForm, OrderFormSaveSuccessDoc } from './sales/components/OrderForm';
...
const Purchases: React.FC = () => {
  ...
  const handleOrderFormSave = async (data: any, asDraft?: boolean, reason?: string, andPay?: boolean) => {
      ...
      const purchaseData: Purchase = {
          ...
          items,
          totalAmount,
          date: data.date || data.orderDate || getDefaultDate(),
          dueDate: data.dueDate || getDefaultDate(),
          status: asDraft ? 'Draft' : 'Ordered',
          reference: data.referenceDoc || data.reference || '',
          notes: data.notes || '',
          paymentStatus: andPay ? 'Paid' : 'Unpaid',
          paidAmount: andPay ? totalAmount : 0,
      };
      if (isEditing) {
          await updatePurchase(purchaseData);
          notify(`Bill ${purchaseId} updated successfully`, 'success');
      } else {
  ...
  const handleEditOrder = (po: Purchase) => {
      ...
      setEditingPurchase({
          ...po,
          customerName: supplier?.name || po.supplierId,
          customerId: po.supplierId,
      });
      setShowNewBillModal(true);
  };
...
.nav-tilde{color:var(--teal-400);margin:0 4px;opacity:.6}
      ...
      {showNewBillModal && (
          <div
              className="absolute inset-0 z-50 bg-slate-50 overflow-y-auto custom-scrollbar p-4 md:p-6"
              onClick={(e) => {
          ...
          >
              <div className="animate-in zoom-in-95 duration-200" role="dialog" aria-modal="true" style={{position:'relative',background: paper,border:`1.4px solid ${hairline}`,borderRadius:14,overflow:'hidden',display:'flex',flexDirection:'column',height:'100%',minHeight:'calc(100vh - 32px)'}}>
                  <div style={{position:'absolute',top:0,left:0,right:0,height:4,zIndex:10,background:'linear-gradient(90deg, #146b60, #3fa294 40%, #d99a3f 100%)'}} />
                  <div className="flex items-center justify-between px-6 pt-4 shrink-0">
                      <h2 className="font-['DM_Serif_Display',serif] text-[27px] leading-[1.15] text-[#23282A]">
                          {editingPurchase ? 'Edit Bill' : 'New Bill'}
                      </h2>
                      <button
                          onClick={() => { setShowNewBillModal(false); setEditingPurchase(null); }}
                  ...
                  <div className="flex-1 min-h-0 overflow-y-auto px-6 pt-4">
                      <OrderForm
                          ...
                          onSave={handleOrderFormSave}
                          onCancel={() => { setShowNewBillModal(false); setEditingPurchase(null); }}
                      />
                  </div>
              </div>
          </div>
      ...
      <div className="mb-5 flex justify-between items-center shrink-0">
         <div className="flex items-center gap-3">
           ...
           <button
             data-create-bill
             onClick={() => { setEditingPurchase(null); setShowNewBillModal(true); }}
             className="purchases-btn-primary"
           >
