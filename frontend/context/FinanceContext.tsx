...
const handleOpenInventory = async () => {
  ...
  const runPayroll = async (month: string, date: string, employeesToPay: Employee[]) => {
      ...
      await postJournalEntry([{
          ...
          debitAccountId: gl.defaultLaborWagesAccount || '52100', // Labor Wages
          creditAccountId: gl.bankAccount || '11210',
          amount: totalBasic,
          referenceId: runId,
          reconciled: false
      }]);
  ...
  /**
   * Returns the invoice's permanent verification token, issuing + persisting
   * one first for pre-token invoices (safe backfill through the normal save
   * path). Used by verification-link actions and the on-open backfill.
   */
  ...
  const postZReportToLedger = async (report: ZReport, targetAccountId: string) => {
      try {
          ...
          await postJournalEntry([{
              ...
              creditAccountId: gl.cashDrawerAccount || '11110',
