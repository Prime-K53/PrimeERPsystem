...
const portalService = {
  ...
  async getReferralCode(portalUserId, customerId) {
    ...
    if (customer && customer.referralCode) {
      return { referralCode: customer.referralCode, shareMessage: null };
    }
    if (customer) {
      try {
        const code = await referralService.generateReferralCode();
        await portalAuthService.syncCustomerPortalData(customerId, { referralCode: code });
        return { referralCode: code, shareMessage: null };
      } catch (err) {
        console.warn('[Portal] getReferralCode: failed to generate/store code:', err?.message);
      }
    }
  ...
  async getSupportArticles() {
    try {
      ...
      console.warn('[Portal] getSupportArticles failed:', err?.message);
      return [];
    }
  },
  ...
  async getSupportArticle(slug) {
    try {
      const rows = await repo.getAll('support_articles', { slug: `eq.${slug}`, limit: 1 });
      const a = rows[0];
      if (!a) return null;
      return {
        id: a.id,
      ...
      console.warn('[Portal] getSupportArticle failed:', err?.message);
      return null;
    }
  },
